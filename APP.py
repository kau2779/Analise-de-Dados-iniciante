...
if not df_filtrado.empty:
    genero_geral = df["Genêro"].mode()[0]
    idade_geral = df["Idade"].mode()[0]
    treino_geral = df["Tipo_Treino"].mode()[0]
else:
...
with col_graf1:
    if not df_filtrado.empty:
        genero_contagem = df["Genêro"].value_counts().reset_index()
...
with col_graf2:
    if not df_filtrado.empty:
        idade_contada = df["Idade"].value_counts().reset_index()
...
with col_graf3:
    if not df_filtrado.empty:
        treino_contagem = df["Tipo_Treino"].value_counts().reset_index()
...
with col_graf4:
    if not df_filtrado.empty:
        presença_contagem = df["Status_Presença"].value_counts().reset_index()
